...
$$
	...
	\lambda\psi(\rho),
$$ 
where \(\rho\) is the same dimensionless variable from \autoref{subsec:23}, the frequency \(\omega_{r}=\frac{1}{4}\frac{mk}{\hbar^{2}}\alpha^{4}\), \(\alpha\) is chosen such that
...
To solve the eigenvalues of the problem outlined in \autoref{subsec:23}, \hyperref[A:3]{"oneelectron.py"} was written.
